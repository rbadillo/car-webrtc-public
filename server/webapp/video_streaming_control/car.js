...
function enableDisableKeyboard() {

	if(isKeyboardEnabled == false){
		$('#keyboard').removeAttr('disabled').html("Disable Keyboard")
		isKeyboardEnabled = true;
	}else{
		$('#keyboard').removeAttr('disabled').html("Enable Keyboard")
...
function pingRPI() {
	socket.emit('check_rpi_connectivity_ping','ping');
	console.log("Ping Button")
}
...
$(document).on('keydown', function(event) {
	...
	if(isKeyboardEnabled){
		...
		socket.emit('rpi_car_control',event.keyCode);
	}else{
...
function registerUsername() {
	if($('#username').length === 0) {
		...
		// Websocket Configuration
		var server_url = websocket_server_url +"?username="+username
	...
	sfutest.createOffer(
		{
			...
			media: { audioRecv: false, videoRecv: false, audioSend: useAudio, videoSend: false, video: "hires", data: true},	// Publishers are sendonly
